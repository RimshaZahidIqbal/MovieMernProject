...
export default function MovieSection({ shows = [], loading = false }) {
    const skeletons = new Array(6).fill(0); // Change 6 based on grid cols
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            {loading ? (
                skeletons.map((_, i) => (
                    <div
                        key={i}
                        className="h-96 w-full bg-gray-800 animate-pulse rounded-xl"
                    ></div>
                ))
            ) : (
                shows.map(show => (
